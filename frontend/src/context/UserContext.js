...
export function UserContextProvider({ children }) {
  ...
  async function signup(nameBody, emailBody, passwordBody) {
    ...
    await axios
      .post("http://localhost:8080/users", registerData) //sends info tu server via POST
      .then((response) => {
  ...
  async function updateUser({ email, password = "", name = "" }) {
    ...
    await axios
      .put(`http://localhost:8080/users/${currentUser.uid}`, updatedUser) //sends info tu server via PUT
      .then((response) => {
        console.log(response);
        setIsLoading(false);
      })
  ...
  async function currentUserFromToken() {
    ...
    const googleUser = JSON.stringify(localStorage.getItem("googleUser"));
    if (googleUser !== "null") {
      setGoogleUser(true);
      setIsUserLogged(true);
      setCurrentUser(JSON.parse(localStorage.getItem("googleUser")));
    }
    //decodes token
    if (token !== "null") {
      setIsUserLogged(true);
  ...
  function handleCredentialResponse(response) {
    ...
    fetch("http://localhost:8080/auth/google", {
      ...
      .then((resp) => {
        ...
        localStorage.setItem("googleUser", loggedUser);
        setIsUserLogged(true);
